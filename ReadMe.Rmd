...
`hoopR` so this report still knits even if an API is down. The CSV lives in
`data/` in this repo.
...
relative to this file, so it knits as long as that CSV is committed in the
same repo under `data/`. It only uses CRAN packages from this course
(`dplyr`, `tidyr`, `readr`, `stringr`, `ggplot2`, `knitr`) and doesn't need
network access to knit.
